...
data class StreamStartData(
    ...
    @SerializedName("stream_url") val streamUrl: String = ""          // ★ 患者端上报的摄像头 MJPEG 地址
)
...
// --- (q) Stream Stop 停止推流请求 ---
data class StreamStopData(
    @SerializedName("stream_type") val streamType: String = "video"  // "video" | "audio"
)

// --- (r) Stream Status 推流状态（服务器 → 端） ---
data class StreamStatusData(
    @SerializedName("stream_type") val streamType: String = "video",
    @SerializedName("status") val status: String = "stopped",       // "live" | "stopped" | "error"
    @SerializedName("url") val url: String = ""
)
...
object MessageType {
    ...
    const val STREAM_STATUS = "stream_status"
}
